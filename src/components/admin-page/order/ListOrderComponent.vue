  ...
  methods: {
    async fetchDataOrder(page = 1) {
      try {
        ...
        const response = await api.get(`/admin/order/list-order?`, { params });
    ...
    async setOrderEdit(order) {
      try {
        ...
        const response = await api.get(`/admin/order/edit-order/${orderID}`);
    ...
    async updateOrderStatus(orderID) {
      try {
        ...
        const response = await api.post(`/admin/order/edit-order/${orderID}`, {
          order_status: status,
        });
