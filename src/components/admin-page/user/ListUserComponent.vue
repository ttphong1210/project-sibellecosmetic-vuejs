              ...
              <tr v-for="item in account" :key="item.id">
                ...
                <td>
                  <img
                    ...
                    :src="`http://192.168.2.1:8080/storage/account-avatar/${item.image}`"
                    alt=""
                  />
                </td>
                <td>{{ item.email }}</td>
...
export default {
  ...
  methods: {
    async fetchDataRoles() {
      try {
        const response = await api.get("admin/account-auth/roles");
    ...
    async fetchAccountAuth() {
      try {
        ...
        const response = await api.get("admin/account-auth/list");
    ...
    hanldeEditAccount(account) {
      this.setEditAccount = {
        ...account,
        roles: account.roles.length ? account.roles[0].id : null,
      };
      this.avatarFile = account.image;
      this.avatarPreview = `http://192.168.2.1:8080/storage/account-avatar/${this.avatarFile}`;
    },
    ...
    async sendToSeverEditAccount(accountID) {
      ...
      try {
        ...
        const response = await api.post(
          `admin/account-auth/edit/${accountID}`,
          formData,
          {
            headers: {
